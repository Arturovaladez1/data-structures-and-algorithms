...
const count = (target, input) => {
  // Solution code here...
  const occurrences = input.reduce((count, subArr) => {
    const filteredArr = subArr.filter(num => num === target);
    return count + filteredArr.length;
  }, 0);
...
const totalSum = (input) => {
  // Solution code here...
  const sum = input.reduce((total, subArr) => {
    const subArrSum = subArr.reduce((subTotal, num) => subTotal + num, 0);
    return total + subArrSum;
  }, 0);
